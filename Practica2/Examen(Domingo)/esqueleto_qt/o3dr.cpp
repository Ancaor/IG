...
/*********************************************************************************
Funcion que realiza la revolucion por barrido respecto al eje y dado un angulo
de inicio y otro de fin, util para generar secciones.
*********************************************************************************/

void O3DR::revolucion_por_barrido_y(float angulo_inicial,float angulo_final)
{
    int i,j;
    float alpha;
    int tamanio = vertices.size();

    alpha = angulo_inicial;
    for(j=0;j<tamanio;j++){
        float x_rotado= ((vertices[j].x) * cos(alpha) + (vertices[j].z) * sin(alpha));
        float z_rotado= (-(vertices[j].x)*sin(alpha) + (vertices[j].z)*cos(alpha) );
        _vertex3f rotado = _vertex3f(x_rotado,vertices[j].y,z_rotado);
    ...
    for(i=1;i <= SECCIONES ; i++){
        alpha = (i*(angulo_final-angulo_inicial))/SECCIONES; // 2.0*M_PI
        for(j=0;j<tamanio;j++){
            float x_rotado= ((vertices[j].x) * cos(alpha) + (vertices[j].z) * sin(alpha));
            float z_rotado= (-(vertices[j].x)*sin(alpha) + (vertices[j].z)*cos(alpha) );
            _vertex3f rotado = _vertex3f(x_rotado,vertices[j].y,z_rotado);
...
{
    SECCIONES--; // Para contar con el perfil
    encontrar_tapas();
    revolucion_por_barrido_y(2*M_PI);
...
/*********************************************************************************
Funcion que genera el objeto por revolucion para unos angulos dados:
    1- primero encuentra las tapas
    2- despues quita las tapas si tiene y realiza la revolucion por barrido al
       cuerpo del objeto
    3- si el cuerpo tiene mas de 2 puntos puede generar sus caras
...
void O3DR::generar_objeto_de_revolucion(float angulo_inicial,float angulo_final)
{
    SECCIONES--; // Para contar con el perfil
    encontrar_tapas();

    float angulo_inicial_radianes = angulo_inicial*(M_PI/180.0);
    float angulo_final_radianes = angulo_final*(M_PI/180.0);

    revolucion_por_barrido_y(angulo_inicial_radianes,angulo_final_radianes);
